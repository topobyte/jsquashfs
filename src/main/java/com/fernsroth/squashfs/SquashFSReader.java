...
public final class SquashFSReader {
	 ...
	 * logging.
	 */
	private static Log log = LogFactory.getLog(SquashFSReader.class);
	...
	private Directory dir_scan(String name, long start_block, long offset)
		...
		if (dir.dirs != null) {
			for (dir_ent dirent : dir.dirs) {
				log.trace("dir_scan: name " + dirent.name + ", start_block "
						+ dirent.start_block + ", offset " + dirent.offset
						+ ", type " + dirent.type);
	...
	private SymLink read_symlink(String name, long start_block, long offset)
		...
		log.trace("create_inode: start " + start + ", offset " + offset);
		...
		log.trace("create_inode: symlink, symlink_size " + inodep.symlink_size);
	...
	private SFSSquashedFile read_file(String name, long start_block, long offset)
		...
		log.trace("create_inode: regular file, file_size " + inode.file_size
				+ ", blocks " + blocks);
	...
	private dir squashfs_openddir(long block_start, long offset)
		...
		log.trace("squashfs_opendir: inode start block " + block_start
				+ ", offset " + offset);
		...
		if (log.isTraceEnabled()) {
			log.trace("data \n"
					+ EasyIOFormatter.print(new ByteArrayInputStream(
							inode_table_bytes, baOffset, baLength)));
		}
		...
		while (bytes < size) {
			...
			log
					.trace("squashfs_opendir: Read directory header @ byte position "
							+ bytes + ", " + dir_count + " directory entries");
			...
			while (dir_count-- != 0) {
				...
				if (log.isTraceEnabled()) {
					log
							.trace("squashfs_dir_entry \n"
									+ EasyIOFormatter
											.print(new ByteArrayInputStream(
				...
				log.trace("squashfs_opendir: directory entry " + dire.name
						+ ", inode " + dirh.start_block + ":" + dire.offset
						+ ", type " + dire.type);
	...
	private void uncompress_directory_table(long start, long end)
			throws IOException {
		while (start < end) {
			log.trace("uncompress_directory_table: reading block " + start);
	...
	private void uncompress_inode_table(long start, long end)
			throws IOException {
		while (start < end) {
			log.trace("uncompress_inode_table: reading block " + start);
	...
	private void read_super() throws EasyIOException, IOException {
		...
		log.debug("superblock.s_magic:" + this.superBlock.s_magic);
		log.debug("superblock.inodes:" + this.superBlock.inodes);
		log.debug("superblock.bytes_used_2:" + this.superBlock.bytes_used_2);
		log.debug("superblock.uid_start_2:" + this.superBlock.uid_start_2);
		log.debug("superblock.guid_start_2:" + this.superBlock.guid_start_2);
		log.debug("superblock.inode_table_start_2:"
				+ this.superBlock.inode_table_start_2);
		log.debug("superblock.directory_table_start_2:"
				+ this.superBlock.directory_table_start_2);
		log.debug("superblock.s_major:" + this.superBlock.s_major);
		log.debug("superblock.s_minor:" + this.superBlock.s_minor);
		log.debug("superblock.block_size_1:" + this.superBlock.block_size_1);
		log.debug("superblock.block_log:" + this.superBlock.block_log);
		log.debug("superblock.flags:" + this.superBlock.flags);
		log.debug("superblock.no_uids:" + this.superBlock.no_uids);
		log.debug("superblock.no_guids:" + this.superBlock.no_guids);
		log.debug("superblock.mkfs_time:" + this.superBlock.mkfs_time);
		log.debug("superblock.root_inode:" + this.superBlock.root_inode);
		log.debug("superblock.block_size:" + this.superBlock.block_size);
		log.debug("superblock.fragments:" + this.superBlock.fragments);
		log.debug("superblock.fragment_table_start_2:"
				+ this.superBlock.fragment_table_start_2);
		log.debug("superblock.bytes_used:" + this.superBlock.bytes_used);
		log.debug("superblock.uid_start:" + this.superBlock.uid_start);
		log.debug("superblock.guid_start:" + this.superBlock.guid_start);
		log.debug("superblock.inode_table_start:"
				+ this.superBlock.inode_table_start);
		log.debug("superblock.directory_table_start:"
				+ this.superBlock.directory_table_start);
		log.debug("superblock.fragment_table_start:"
				+ this.superBlock.fragment_table_start);
		log.debug("superblock.unused:" + this.superBlock.unused);
	}
	...
	private void read_fragment_table() throws IOException, EasyIOException {
		...
		log.trace("read_fragment_table: " + this.superBlock.fragments
				+ " fragments, reading " + indexes + " fragment indexes from "
				+ this.superBlock.fragment_table_start);
	...
	private byte[] read_block(long start, long[] next) throws IOException {
		...
		log
				.trace("read_block: block "
						+ start
						+ ", "
						+ squashfs_constants.SQUASHFS_COMPRESSED_SIZE(c_byte)
						+ " "
						+ (squashfs_constants.SQUASHFS_COMPRESSED(c_byte) ? "compressed"
								: "uncompressed") + " bytes");
	...
	private byte[] read_data_block(long start, long size) throws IOException {
		...
		this.source.seek((int) start);
		if (this.source.read(readdata) != readdata.length) {
	...
	private byte[] read_fragment(squashfs_fragment_entry fragment_entry)
			throws IOException {
		log.trace("read_fragment: reading fragment "
				+ fragment_entry.start_block);
	...
	public void writeFile(SFSSquashedFile squashFile, OutputStream destFile)
		...
		log.trace("write_file: regular file, blocks " + squashFile.getBlocks());
